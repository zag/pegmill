...
var exprArgName = require("./expr-arg-name");
...
function cloneExpression(expr, paramMap) {
  switch (expr.type) {
    case "rule_ref": {
      if (paramMap.hasOwnProperty(expr.name)) {
        var subst = paramMap[expr.name];
        /* Backward compat: accept plain string (rule name) or expression node. */
        if (typeof subst === "string") {
          subst = { type: "rule_ref", name: subst };
        }
        return cloneExpression(subst, {});
      }
      var node = { type: "rule_ref", name: expr.name };
      if (expr.location) { node.location = expr.location; }
      if (expr.args) {
        node.args = expr.args.map(function(arg) {
          if (paramMap.hasOwnProperty(arg.name)) {
            var s = paramMap[arg.name];
            if (typeof s === "string") { s = { type: "rule_ref", name: s }; }
            var n = (s.type === "rule_ref" && !s.args)
              ? s.name
              : exprArgName(s);
            return { name: n, value: s };
          }
          return { name: arg.name, value: arg.name };
        });
      }
      return node;
    }
